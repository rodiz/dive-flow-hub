-- First, drop the policies that depend on student_id
DROP POLICY IF EXISTS "Users can view their own dives" ON public.dives;
DROP POLICY IF EXISTS "Users can view equipment usage for their dives" ON public.equipment_usage;
...
-- Remove student_id from dives table
ALTER TABLE public.dives DROP COLUMN student_id;
...
-- Recreate the dives policies without student_id dependency
CREATE POLICY "Instructors can view their own dives" 
ON public.dives 
FOR SELECT 
USING (instructor_id = auth.uid());

CREATE POLICY "Students can view dives they participate in" 
ON public.dives 
FOR SELECT 
USING (EXISTS (
  SELECT 1 FROM public.dive_participants dp 
  WHERE dp.dive_id = dives.id AND dp.student_id = auth.uid()
));

-- Update equipment usage policy
CREATE POLICY "Users can view equipment usage for their dives" 
...
USING (EXISTS (
  ...
  AND (d.instructor_id = auth.uid() 
    OR EXISTS (
      SELECT 1 FROM public.dive_participants dp 
      WHERE dp.dive_id = d.id AND dp.student_id = auth.uid()
    )
  )
));
